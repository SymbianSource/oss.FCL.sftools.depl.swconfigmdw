...
EXPORT_C void RSdbTest::EndL(TInt aExitValue)
	...
	iLastError = aExitValue;
	WriteExitValueL();
	iTest.End();
	iEnded = ETrue;
	}

EXPORT_C void RSdbTest::AsyncEndL()
	{
	if (iEnded)
		{
		return;
		}
	User::WaitForRequest(iTestStatus);
	
	if(iTestStatus.Int() != KErrNone)
		iLastError = iTestStatus.Int();
...
EXPORT_C TInt RSdbTest::GetLastError()
	{
	 return iLastError;
	}
...
EXPORT_C void RSdbTest::HandleTestResults(TInt aExitVal)
{
	iLastError = aExitVal;
	TRequestStatus *status = &iTestStatus; 
