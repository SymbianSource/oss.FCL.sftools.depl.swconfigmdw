...
//  Class Definitions
class RSdbTest : public MTestCaseObserver
	{
public:
	/**
	 * Default constructor.
	...
	/**
	 * End the test after async handler completes. This function writes out the last error set in 
	 * handler to the test result file which will be read by Java wrapper.
	 */
	IMPORT_C void AsyncEndL();
	...
	/**
	 * Return last error (test exit value)
	 * @return system wide error code
	 */
	IMPORT_C TInt GetLastError();
	
	/**
	* MTestCaseObserver implementaion
	*/
	IMPORT_C void HandleTestResults(TInt aExitVal);
	
private:
	
	/**
	 * Write test status code to file
	 */
	void RSdbTest::WriteExitValueL();
